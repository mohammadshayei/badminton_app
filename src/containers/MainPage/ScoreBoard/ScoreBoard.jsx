...
const ScoreBoard = ({ disable, setDisable }) => {
  ...
  const [undoLastPoint, setUndoLastPoint] = useState(false);
  const [chooseServeTriger, setChooseServeTriger] = useState(-1);
  ...
  const removeEventFromStack = () => {
    if (info.events[info.events.length - 1].type === "score") {
      if (
        (info.team1.score === 10 && info.team2.score <= 10) ||
        (info.team2.score === 10 && info.team1.score <= 10)
      )
        setBreakTime(0);
      if (
        (info.team1.score === 11 && info.team2.score <= 11) ||
        (info.team2.score === 11 && info.team1.score <= 11)
      ) {
  ...
  const createNewSet = async () => {
    ...
    const payload = {
      ...
      teamA: {
        ...
        players: info.team1.players.map((item) => {
      ...
      teamB: {
        ...
        players: info.team2.players.map((item) => {
          return { player: item.id };
        }),
      },
    };
    ...
    if (resultCreateSet.success) {
      ...
      setDialog(
        <ErrorDialog type="error">{resultCreateSet.error}</ErrorDialog>
      );
    }
  };
  const endSet = async (teamName) => {
    ...
    let payload = {
      ...
      teamB: {
        ...
        setWon: teamName === "team2" ? true : false,
      },
    };
  ...
  useEffect(() => {
    ...
    switch (info.team1.score) {
      ...
      case maxPoint:
        setWinPoint(null);
        if (info.team1.setWon < 1) setSetOverConfirmation(true);
        else setOverConfirmed("team1");
  ...
  useEffect(() => {
    ...
    switch (info.team2.score) {
      ...
      case maxPoint:
        setWinPoint(null);
        if (info.team2.setWon < 1) setSetOverConfirmation(true);
        else setOverConfirmed("team2");
  ...
  useEffect(() => {
    (async () => {
      ...
      if (info.team1.setWon === 2) setTeamWon("team1");
      else if (info.team2.setWon === 2) setTeamWon("team2");
    })();
    ...
    if (breakTime === 2 || breakTime === 3) {
      ...
      return () => {
        ...
        if (info.team1.players.length > 1 && breakTime === 3) {
          setChooseServeTriger((prevState) =>
            prevState === -1 ? 0 : prevState === 0 ? 1 : 0
          );
        }
      };
    }
  ...
  useEffect(() => {
    if (chooseServeTriger < 0 || undoLastPoint) return;
  ...
  useEffect(() => {
    (async () => {
      if (
        ...
        (info.events[info.events.length - 1].content === "Dis" ||
          info.events[info.events.length - 1].content === "Ret")
      ) {
        let teamWon = "team1";
        info.team1.players.forEach((player) => {
          if (player.id === info.events[info.events.length - 1].by)
            teamWon = "team2";
        });
        setTeamWon(teamWon);
        setWinPoint(null);
  ...
  useEffect(() => {
    ...
    if (
      info.team1.players.findIndex(
        (item) => item.id === info.events[index].by
      ) > -1
    ) {
  ...
  useEffect(() => {
    ...
    if (info.events.length === 0) {
      if (info.team1.server > 0) {
        temp = info.team1.isRightTeam ? "down-left" : "up-right";
      } else {
        temp = info.team2.isRightTeam ? "down-left" : "up-right";
      }
      setServerDirection(temp);
    ...
    info.team1.server,
    info.team2.server,
    info.team1.isRightTeam,
    info.team2.isRightTeam,
  ]);
  ...
  return (
    ...
    >
      ...
      {(teamWon === "team1" || teamWon === "team2") && (
        <Modal
          show={(teamWon === "team1" || teamWon === "team2") && true}
      ...
      <div
        className={`main-scoreboard ${info.team1.isRightTeam && "reverse"}`}
        style={{
      ...
      >
        {!disable && (
          <div
            className={`arrow ${serverDirection}`}
            style={{
              fontSize: `${Math.min(
        ...
        {!loading &&
          (info ? (
            info.team1.players.length > 0 && info.team2.players.length > 0 ? (
        ...
        <div className="win-point" style={{ opacity: winPoint ? 1 : 0 }}>
          {winPoint}
        </div>
        {disable &&
          breakTime === 0 &&
        ...
        {(breakTime === 2 || breakTime === 3) && (
          <div
            className={`break-timer ${twentySeconds && "alert"}`}
            style={{
              fontSize: `${Math.min(
                window.innerWidth * 0.06,
      ...
      {(!disable || breakTime !== 0) && (
        <>
          <div
            className="action-buttons action-btn-right"
            style={{
              opacity: info.foulHappend ? 0 : 1,
              zIndex: info.foulHappend && -1,
            }}
          ...
          <div
            className="action-buttons action-btn-left"
            style={{
              opacity: info.foulHappend ? 0 : 1,
              zIndex: info.foulHappend && -1,
            }}

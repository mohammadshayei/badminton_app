...
const Games = ({ tournamentId, createAccess, gameDate }) => {
  ...
  const onChangeDatePicker = async (e) => {
    ...
    try {
      ...
      setDialog(
        <ErrorDialog type={result.success ? "success" : "error"}>
          {result.data.message}
        </ErrorDialog>
      );
      if (!result.success) {
      ...
      setDateLoading(false);
      setDialog(
        <ErrorDialog type="error">{stringFa.error_occured}</ErrorDialog>
      );
    }
  };
  ...
  const selectDay = async (key) => {
    ...
    try {
      ...
      const result = await dynamicApi(
        payload,
        token,
        "get_free_ranking_day_info"
      );
      if (!result.success) {
        setDialog(
          <ErrorDialog type="error"> {result.data.message}</ErrorDialog>
        );
      } else {
  ...
  const onChangeGym = async (e) => {
    ...
    try {
      const result = await dynamicApi(
        { tournamentId, itemId: e.id },
        token,
        "set_gym"
      );
      if (!result.success) return;
      setLandNumbers(
        result.data.landNumbers.map((item, index) => {
          return {
            text: item.number,
            id: index,
          };
  ...
  const toggle = (gameKey) => {
    ...
    updatedGames[gameIndex].officialsOpen =
      !updatedGames[gameIndex].officialsOpen;
    ...
    inputGames.forEach((item) => {
      updatedGames.push({
        ...
        officials: {
          ...
          serviceJudge: [
            {
              _id: item.game.service_referee
                ? item.game.service_referee._id
                : "",
              value: item.game.service_referee
                ? item.game.service_referee.username
                : "",
            },
          ],
        },
  ...
  const onSave = async (gameKey) => {
    ...
    if (
      !updatedGames[gameIndex].court.value &&
      updatedGames[gameIndex].officials.umpire[0]._id
    ) {
      setDialog(
        <ErrorDialog type="error">شماره زمین را باید تعیین کنید</ErrorDialog>
      );
      updatedGames[gameIndex].loading = false;
      setGames(updatedGames);
      setItemLoading({ type: "", content: "" });
    ...
    let path,
      payload = {
        gameType:
          updatedGames[gameIndex].players.b.length === 2 ? "double" : "single",
    ...
    } else path = "create_free_ranking_game";
      ...
      if (fetchedTournament.success) {
        setDialog(
          <ErrorDialog type="success">
            {fetchedTournament.data.message}
          </ErrorDialog>
        );
        if (!updatedGames[gameIndex].fetched) {
        ...
        else updatedGames[gameIndex].status = 0;
      } else {
        setDialog(
  ...
  const onRemove = async (gameKey) => {
    ...
    try {
      let fetchedTournament = await dynamicApi(
        payload,
        token,
        "remove_game_from_tournament"
      );
      if (fetchedTournament.success) {
        setDialog(
          <ErrorDialog type="success">
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const fetchedData = await dynamicApi(
          { id: tournamentId },
          token,
          "get_days_free_ranking_info"
        );
        ...
        setDateValue(
          fetchedData.data.selectedDay.date
            ? fetchedData.data.selectedDay.date
            : ""
        );
        ...
        if (fetchedData.data.selectedGym) {
          setGym({
            id: fetchedData.data.selectedGym._id,
            value: fetchedData.data.selectedGym.title,
          });
          setLandNumbers(
            fetchedData.data.selectedGym.land_numbers.map((item, index) => {
              return {
                text: item.number,
                id: index,
              };
  ...
  return (
    <div className="tournament-games-wrapper">
      {dialog}
